        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className={cn(
              ...
              activeTab === 'chats' ? 'bg-primary text-black' : 'text-muted-foreground hover:text-foreground hover:bg-secondary/40'
            )}
          >
          ...
          <button 
            ...
            className={cn(
              "px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-2 transition-all",
              activeTab === 'broadcast' ? 'bg-primary text-black' : 'text-muted-foreground hover:text-foreground hover:bg-secondary/40'
            )}
          >
          ...
          <button 
            ...
            className={cn(
              "px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-2 transition-all",
              activeTab === 'templates' ? 'bg-primary text-black' : 'text-muted-foreground hover:text-foreground hover:bg-secondary/40'
            )}
          >
                ...
                {filteredChats.length > 0 ? (
                  filteredChats.map((chat) => {
                    ...
                    return (
                      ...
                      >
                        <div className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center text-xs font-bold uppercase text-muted-foreground shrink-0">
            ...
            <div className="flex-1 flex flex-col h-full relative">
              {activeChat ? (
                <>
                    ...
                    <div className="flex items-center gap-3 bg-muted border border-border px-3.5 py-1.5 rounded-lg">
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          activeChat.aiMode ? 'bg-primary' : 'border-border'
                        )}
                      >
                        <div className={cn(
                  ...
                  {/* Message Stream */}
                  <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-background/10">
                      ...
                      return (
                        ...
                        >
                          <div className={cn(
                            ...
                            isLead 
                              ? 'bg-muted/90 text-foreground rounded-tl-none border border-border' 
                              : isAi
            ...
            <div className="w-72 shrink-0 h-full bg-secondary/5 overflow-y-auto p-4 space-y-6">
              {activeChat ? (
                <>
                    ...
                    <div className="border border-border p-3 rounded-lg bg-card text-xs space-y-2">
                      ...
                      {activeLead?.budget && (
                        <div className="text-[10px] text-muted-foreground pt-1">
                          <span className="font-bold block text-muted-foreground">Budget Range</span>
                          <span className="text-foreground font-semibold">{activeLead.budget}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <h4 className="text-[9px] uppercase font-bold tracking-widest text-muted-foreground border-b border-border pb-1">Qualification Templates</h4>
                      ...
                      <div className="space-y-1.5 max-h-32 overflow-y-auto">
                        {scheduledLogs.map(log => (
                          <div key={log.id} className="p-2 border border-border/80 rounded bg-muted text-[9px] space-y-0.5">
                            ...
                            <span className="text-muted-foreground font-mono block">Ends: {log.time}</span>
                          </div>
                        ))}
                      </div>
                    )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {campaigns.map((camp) => (
                <div key={camp.id} className="border border-border rounded-xl p-5 bg-secondary/10 flex flex-col justify-between space-y-4">
                  <div>
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-xs text-foreground truncate max-w-[150px]">{camp.name}</span>
                      <span className={cn(
                  ...
                  {camp.status === 'Completed' && (
                    <div className="grid grid-cols-3 gap-2 text-center bg-muted/60 p-2.5 rounded border border-border/40 text-[10px]">
                      <div>
                        <span className="text-[9px] text-muted-foreground block">Sent</span>
                        <span className="font-bold text-foreground mt-0.5 block">{camp.sentCount}</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-muted-foreground block">Delivered</span>
                        <span className="font-bold text-muted-foreground mt-0.5 block">{camp.deliveredCount}</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-muted-foreground block">Read</span>
                  ...
                  {camp.status === 'Scheduled' && (
                    <div className="flex items-center gap-1.5 p-2 bg-muted border border-border rounded text-[9px] text-muted-foreground">
                ...
                <div key={tpl.id} className="border border-border rounded-xl p-5 bg-secondary/10 flex flex-col justify-between space-y-4">
                  <div>
                    <div className="flex justify-between items-start">
                      ...
                      <span className="px-2 py-0.5 bg-secondary border border-border rounded text-[8px] font-semibold text-muted-foreground">
                        {tpl.category}
                      </span>
                    </div>
                    <p className="text-[11px] text-muted-foreground italic leading-relaxed mt-3 bg-muted/40 p-3 rounded border border-border/40 font-mono">
                      {tpl.body}
                    </p>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/85 backdrop-blur-sm px-4">
          <div className="bg-card border border-border rounded-xl p-6 max-w-sm w-full relative animate-fade-in">
            ...
            <form onSubmit={handleLaunchCampaign} className="mt-4 space-y-4">
              <div>
                <label className="text-[9px] uppercase font-bold text-muted-foreground block mb-1">Campaign Name</label>
                <input 
                  type="text" 
                  required
              ...
              <div>
                <label className="text-[9px] uppercase font-bold text-muted-foreground block mb-1">Select Message Template</label>
                <select
                  required
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/85 backdrop-blur-sm px-4">
          <div className="bg-card border border-border rounded-xl p-6 max-w-sm w-full relative animate-fade-in">
            ...
            <form onSubmit={handleCreateTemplate} className="mt-4 space-y-4">
              <div>
                <label className="text-[9px] uppercase font-bold text-muted-foreground block mb-1">Template Name</label>
                <input 
                  type="text" 
                  required
              ...
              <div>
                <label className="text-[9px] uppercase font-bold text-muted-foreground block mb-1">Body Text Content</label>
                <textarea 
                  required
                  rows={4}
                  ...
                  className="w-full bg-muted border border-border rounded p-2 text-xs text-foreground focus:outline-none focus:border-primary font-mono"
                />
              </div>
